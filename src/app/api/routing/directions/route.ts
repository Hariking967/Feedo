...
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      if (orsResponse.ok) {
        ...
        if (feature) {
          return NextResponse.json({
            points: feature.geometry.coordinates.map((point: number[]) => [point[1], point[0]]),
    ...
    if (!osrmResponse.ok) return NextResponse.json({ error: "No route available" }, { status: 502 });

    const osrmJson = await osrmResponse.json();
    const best = osrmJson.routes?.[0];
    if (!best) return NextResponse.json({ error: "No route geometry" }, { status: 404 });
