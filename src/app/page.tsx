...
import { HomeSidebar } from "@/components/platform/layout/home-sidebar";

function MapLoadErrorFallback() {
  return (
    <div className="h-[58vh] flex items-center justify-center rounded-xl border border-rose-200 bg-rose-50 text-rose-700">
...
function MapLoadingFallback() {
  return (
    <div className="h-[58vh] flex items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-500">
      Loading map...
    </div>
  );
}

const ConsumerNearbyMap = dynamic(
  () => import("@/components/consumer-nearby-map"),
  {
    ssr: false,
  },
);

const PostPaymentTrackerMap = dynamic(
  () =>
    import("@/components/post-payment-tracker-map").catch(
      () => MapLoadErrorFallback,
    ),
  {
    ssr: false,
...
export default function LandingPage() {
  ...
  const [volunteerTransportMode, setVolunteerTransportMode] = useState<
    "bike" | "scooter" | "van" | "truck"
  >("bike");
  const [volunteerCarryingCapacityKg, setVolunteerCarryingCapacityKg] =
    useState(18);
  const [volunteerPreferredZones, setVolunteerPreferredZones] =
    useState("Central Bengaluru");
  const [volunteerLocation, setVolunteerLocation] = useState<Coordinate | null>(
    null,
  );
  const [volunteerTaskFeed, setVolunteerTaskFeed] = useState<
    VolunteerTaskFeedItem[]
  >([]);
  const [volunteerPooledTasks, setVolunteerPooledTasks] = useState<
    VolunteerPooledTask[]
  >([]);
  const [volunteerWeatherAdvisory, setVolunteerWeatherAdvisory] =
    useState<VolunteerWeatherAdvisory | null>(null);
  const [volunteerWorkflowStatusByTask, setVolunteerWorkflowStatusByTask] =
    useState<Record<string, VolunteerWorkflowStatus>>({});
  const [volunteerWorkflowNoteByTask, setVolunteerWorkflowNoteByTask] =
    useState<Record<string, string>>({});
  const [volunteerWorkflowProofByTask, setVolunteerWorkflowProofByTask] =
    useState<Record<string, File | null>>({});
  const [volunteerWorkflowBusyByTask, setVolunteerWorkflowBusyByTask] =
    useState<Record<string, boolean>>({});
  const [isVolunteerTaskFeedLoading, setIsVolunteerTaskFeedLoading] =
    useState(false);
  ...
  const [rankedConsumerFeed, setRankedConsumerFeed] = useState<
    RankedConsumerFeedItem[]
  >([]);
  const [, setRankedConsumerFeedSource] = useState<string>("fallback");
  const [receiverCrisisSignal, setReceiverCrisisSignal] =
    useState<ReceiverCrisisSignal | null>(null);
  const [, setIsRankedConsumerLoading] = useState(false);
  const [consumerNeedTitle, setConsumerNeedTitle] =
    useState("30 meals tonight");
  const [consumerNeedMeals, setConsumerNeedMeals] = useState(30);
  const [consumerNeedFoodPreference, setConsumerNeedFoodPreference] = useState<
    "any" | "veg" | "non_veg" | "dairy" | "bakery" | "rice" | "seafood"
  >("veg");
  const [consumerNeedMealSlot, setConsumerNeedMealSlot] = useState<
    "tonight" | "breakfast" | "lunch" | "dinner" | "custom"
  >("tonight");
  const [consumerNeedUrgency, setConsumerNeedUrgency] = useState<
  ...
  const [receiverNeedHistory, setReceiverNeedHistory] = useState<
    ReceiverNeedRequest[]
  >([]);
  const [isPostingConsumerNeed, setIsPostingConsumerNeed] = useState(false);
  const [consumerNeedMessage, setConsumerNeedMessage] = useState<string | null>(
    null,
  );
  ...
  const [sellerTypeFilter, setSellerTypeFilter] = useState<"all" | SellerType>(
    "all",
  );
  ...
  const [isNeedPostingOpen, setIsNeedPostingOpen] = useState(false);
  ...
  const [isPublishingSupplierListing, setIsPublishingSupplierListing] =
    useState(false);
  const [supplierPublishMode, setSupplierPublishMode] =
    useState<SupplierPublishMode>("standard");
  const [supplierEmergencyWindowMinutes, setSupplierEmergencyWindowMinutes] =
    useState(60);
  const [supplierEmergencyResult, setSupplierEmergencyResult] =
    useState<SupplierEmergencyResult | null>(null);
  const [supplierBulkEventName, setSupplierBulkEventName] = useState(
    "Wedding or Event Surplus",
  );
  const [supplierBulkItems, setSupplierBulkItems] = useState<
    SupplierBulkItemDraft[]
  >([createBulkItemDraft()]);
  const [supplierBulkWindowMinutes, setSupplierBulkWindowMinutes] =
    useState(120);
  const [supplierBulkResult, setSupplierBulkResult] =
    useState<SupplierBulkResult | null>(null);
  const [isPublishingBulkEvent, setIsPublishingBulkEvent] = useState(false);
  const [isTriggeringEmergencyListing, setIsTriggeringEmergencyListing] =
    useState(false);
  const [isResolvingSupplierAddress, setIsResolvingSupplierAddress] =
    useState(false);
  const [supplierRiskPreview, setSupplierRiskPreview] =
    useState<SupplierRiskPreview | null>(null);
  const [isSupplierRiskLoading, setIsSupplierRiskLoading] = useState(false);
  const [supplierListings, setSupplierListings] = useState<
    SupplierListingRecord[]
  >([]);
  const [supplierNeedPrompts, setSupplierNeedPrompts] = useState<
    SupplierNeedPrompt[]
  >([]);
  const [isLoadingSupplierNeedPrompts, setIsLoadingSupplierNeedPrompts] =
    useState(false);
  const [supplierPaymentProfile, setSupplierPaymentProfile] =
    useState<SupplierPaymentProfile | null>(null);
  const [supplierAnalytics, setSupplierAnalytics] =
    useState<SupplierAnalyticsSnapshot | null>(null);
  const [receiverAnalytics, setReceiverAnalytics] =
    useState<ReceiverAnalyticsSnapshot | null>(null);
  const [volunteerAnalytics, setVolunteerAnalytics] =
    useState<VolunteerAnalyticsSnapshot | null>(null);
  const [isRoleAnalyticsLoading, setIsRoleAnalyticsLoading] = useState(false);
  const [workspaceTimeline, setWorkspaceTimeline] = useState<
    LifecycleTimelineItem[]
  >([]);
  const [isWorkspaceTimelineLoading, setIsWorkspaceTimelineLoading] =
    useState(false);
  ...
  const [supplierQrMessage, setSupplierQrMessage] = useState<string | null>(
    null,
  );
  const [isSavingSupplierQr, setIsSavingSupplierQr] = useState(false);

  const [supplierMaxPrice, setSupplierMaxPrice] = useState<number | null>(null);
  const [isEvaluatingMaxPrice, setIsEvaluatingMaxPrice] = useState(false);
  const [supplierPriceWarning, setSupplierPriceWarning] = useState<
    string | null
  >(null);

  useEffect(() => {
    if (mode !== "supplier" || supplierPublishMode !== "standard") return;
    const { foodName, quantity, foodCategory } = supplierForm;
    if (foodName.length < 3) {
      setSupplierMaxPrice(null);
      return;
    }
    const timer = setTimeout(async () => {
      setIsEvaluatingMaxPrice(true);
      try {
        const res = await fetch("/api/food/market-value", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ foodName, quantity, foodCategory }),
        });
        const data = await res.json();
        if (data.marketValue) {
          setSupplierMaxPrice(data.marketValue * 0.5);
        }
      } catch (err) {
        console.error("AI Price fetch error", err);
      } finally {
        setIsEvaluatingMaxPrice(false);
      }
    }, 1500);
    return () => clearTimeout(timer);
  }, [
    supplierForm.foodName,
    supplierForm.quantity,
  ...
  useEffect(() => {
    if (supplierMaxPrice !== null && supplierForm.price > supplierMaxPrice) {
      setSupplierPriceWarning(
        `Max allowed is 50% market value (Rs. ${supplierMaxPrice.toFixed(0)})`,
      );
    } else {
      setSupplierPriceWarning(null);
    }
  }, [supplierForm.price, supplierMaxPrice]);

  const [paymentReferences, setPaymentReferences] = useState<
    SupplierPaymentReference[]
  >([]);
  const [isPaying, setIsPaying] = useState(false);
  const [isCrisisBannerDismissed, setIsCrisisBannerDismissed] = useState(false);

  const [trackingOrder, setTrackingOrder] = useState<TrackingOrder | null>(
    null,
  );
  const [trackingLocation, setTrackingLocation] = useState<Coordinate | null>(
    null,
  );
  const [trackingRoutePoints, setTrackingRoutePoints] = useState<
    Array<[number, number]>
  >([]);
  const [trackingDistanceKm, setTrackingDistanceKm] = useState<number | null>(
    null,
  );
  const [trackingEtaMinutes, setTrackingEtaMinutes] = useState<number | null>(
    null,
  );
  ...
  const [inboundTracking, setInboundTracking] =
    useState<InboundTrackingSnapshot | null>(null);
  const [isInboundTrackingLoading, setIsInboundTrackingLoading] =
    useState(false);
  const showCrisisBanner =
    receiverCrisisSignal?.active && !isCrisisBannerDismissed;

  const lastRouteFetchAtRef = useRef(0);
  const lastRouteOriginRef = useRef<Coordinate | null>(null);
  const lastInboundStageRef = useRef<InboundTrackingSnapshot["stage"] | null>(
    null,
  );
    ...
    const loadWorkspaceTimeline = async () => {
      ...
      try {
        const response = await fetch(
          `/api/lifecycle?limit=6&actorRole=${workspaceLifecycleRole}`,
          {
            cache: "no-store",
  ...
  const loadSupplierListings = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/food/listings?status=active", {
        cache: "no-store",
      });
      if (!response.ok) return;

      const payload = (await response.json()) as {
        listings?: SupplierListingRecord[];
      };
  ...
  const loadRankedConsumerFeed = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/receiver/feed", {
        cache: "no-store",
        headers: receiverAuthHeaders,
      });
      ...
      const rankedFeed = Array.isArray(payload.rankedFeed)
        ? payload.rankedFeed
        : [];
        ...
        setListings((current) => {
          ...
          for (const item of rankedFeed) {
            ...
            const listingFoodType: FoodType =
              item.foodCategory === "non_veg" || item.foodCategory === "seafood"
                ? "non_veg"
                : "veg";
  ...
  const saveVolunteerProfile = useCallback(async () => {
    ...
    receiverAuthHeaders,
    session?.user?.id,
    volunteerCarryingCapacityKg,
    volunteerLocation,
    volunteerPreferredZones,
    volunteerTransportMode,
  ]);
  ...
  const loadVolunteerTaskFeed = useCallback(async () => {
    ...
    try {
      ...
      const taskUrl = params.size
        ? `/api/volunteer/tasks?${params.toString()}`
        : "/api/volunteer/tasks";
      ...
      setVolunteerPooledTasks(
        Array.isArray(payload.pooledTasks) ? payload.pooledTasks : [],
      );
      ...
      if (typeof payload.profile?.carryingCapacityKg === "number") {
        setVolunteerCarryingCapacityKg(
          Math.max(5, Math.round(payload.profile.carryingCapacityKg)),
        );
      }
      ...
      if (
        Array.isArray(payload.profile?.preferredZones) &&
        payload.profile.preferredZones.length
      ) {
  ...
  const submitVolunteerWorkflowUpdate = useCallback(
    async (task: VolunteerTaskFeedItem) => {
      ...
      setVolunteerWorkflowBusyByTask((current) => ({
        ...current,
        [task.taskId]: true,
      }));
      ...
      try {
        const proofImageDataUrl = proofFile
          ? await readImageAsDataUrl(proofFile)
          : undefined;
        ...
        if (!response.ok) {
          ...
          setVolunteerMessage(
            payload?.error ?? "Unable to update workflow stage.",
          );
          return;
        }

        setVolunteerMessage(
          `Workflow updated for ${task.title}: ${status.replace("_", " ")}.`,
        );
  ...
  useEffect(() => {
    ...
    setConsumerNeedWindowStart(
      (current) => current || now.toISOString().slice(0, 16),
    );
    setConsumerNeedWindowEnd(
      (current) => current || end.toISOString().slice(0, 16),
    );

    const syncReceiverPreferences = async () => {
      const fallbackLocation = listings[0]?.location ?? {
        lat: 12.9716,
        lng: 77.5946,
      };
    ...
    void Promise.all([
      syncReceiverPreferences(),
      loadRankedConsumerFeed(),
      loadReceiverNeedHistory(),
    ]);
    ...
    const loadRoleAnalytics = async () => {
      ...
      try {
        if (mode === "supplier") {
          ...
          if (response.ok) {
            const payload =
              (await response.json()) as SupplierAnalyticsSnapshot;
        ...
        if (mode === "consumer") {
          ...
          if (response.ok) {
            const payload =
              (await response.json()) as ReceiverAnalyticsSnapshot;
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setVolunteerLocation(
          (current) => current ?? { lat: 12.9716, lng: 77.5946 },
        );
      },
  ...
  const postConsumerNeed = async () => {
    ...
    try {
      const response = await fetch("/api/receiver/needs", {
        ...
        body: JSON.stringify({
          needTitle:
            consumerNeedTitle.trim() || `${consumerNeedMeals} meals needed`,
      ...
      if (!response.ok) {
        setConsumerNeedMessage(
          payload?.error ?? "Unable to post receiver need.",
        );
        return;
      }
  ...
  const recommendationByListingId = useMemo(() => {
    ...
    const localScored = listings.map((listing) => {
      const distanceScore = Math.max(
        0,
        1 - listing.distanceKm / Math.max(1, maxDistanceFilter),
      );
      const priceScore = Math.max(
        0,
        1 - listing.unitPrice / Math.max(1, maxPriceFilter),
      );
      const deliveryBoost = listing.deliveryAvailable ? 0.08 : 0;
      const score = Math.round(
        Math.min(1, distanceScore * 0.55 + priceScore * 0.37 + deliveryBoost) *
          100,
      );
    ...
    for (const item of localScored) {
      ...
      map.set(item.listingId, {
        ...
        source: "local",
        detail:
          "Fallback recommendation based on distance, price, and availability.",
      });
      nextRank += 1;
    }
  ...
  const resolveSupplierPickupAddress = async () => {
    ...
    if (!query) {
      setSupplierMessage(
        "Enter pickup address text before resolving location.",
      );
      return;
    }
    ...
    try {
      ...
      if (!candidates.length) {
        setSupplierMessage(
          "No location match found. Try a more specific address.",
        );
        return;
      }
      ...
      setSupplierMessage(
        "Pickup address resolved. You can adjust the pin on map.",
      );
    } catch {
  ...
  useEffect(() => {
    ...
    if (
      now - lastRouteFetchAtRef.current < 20000 &&
      movedSinceLastFetch < 0.12
    ) {
      return;
    }
    ...
    const updateLiveRoute = async () => {
      ...
      try {
        const response = await fetch(
          `/api/logistics/route?${params.toString()}`,
          { cache: "no-store" },
        );
        if (!response.ok) {
        ...
        setTrackingError(
          (current) =>
            current ??
            "Live route updates are delayed. Distance is based on direct location.",
        );
      }
    };
  ...
  const loadInboundTracking = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/receiver/inbound-tracking?${params.toString()}`,
        {
          cache: "no-store",
          headers: receiverAuthHeaders,
        },
  ...
  const consumerListings = useMemo(() => {
    const bySearchAndFilter = listings.filter((listing) => {
      const searchMatch = listing.dish
        .toLowerCase()
        .includes(searchText.toLowerCase());
      const priceMatch = listing.unitPrice <= maxPriceFilter;
      const distanceMatch = listing.distanceKm <= maxDistanceFilter;
      const foodMatch =
        foodTypeFilter === "all" || listing.foodType === foodTypeFilter;
      const sellerMatch =
        sellerTypeFilter === "all" || listing.sellerType === sellerTypeFilter;
  ...
  const handleSupplierQrFileChange = async (
    ...
    try {
      ...
      if (!imageDataUrl.startsWith("data:image/")) {
        setSupplierQrMessage(
          "Unsupported image format. Please upload a valid image.",
        );
        setSupplierQrDraft(null);
        setSupplierQrFileName("");
        return;
      ...
      setSupplierQrMessage(
        "QR ready to upload. Save to update your supplier profile.",
      );
    } catch {
  ...
  const saveSupplierQrProfile = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          payload?.error ?? "Unable to save supplier QR profile.",
        );
      }
      ...
      setSupplierQrMessage(
        error instanceof Error
          ? error.message
          : "Unable to save supplier QR profile.",
      );
    } finally {
  ...
  const resolveSupplierPaymentReferences = async () => {
    ...
    const referencesFromApi = await Promise.all(
      Array.from(uniqueSupplierMap.entries()).map(
        async ([supplierId, listing]) => {
          try {
            const response = await fetch(
              `/api/payment-profile?supplierId=${encodeURIComponent(supplierId)}`,
              {
                cache: "no-store",
              },
            );
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      const [primaryItem] = [...cartItems].sort(
        (a, b) => a.listing.distanceKm - b.listing.distanceKm,
      );
      if (primaryItem) {
        setTrackingOrder({
          id: `order-${Date.now()}`,
      ...
      setMode("consumer");
      setPaymentMessage(
        "Payment initiated. Supplier payment reference is now visible in the tracking view.",
      );
    } catch {
  ...
  const submitSupplierListing = async () => {
    ...
    if (!supplierForm.pickupLocation) {
      setSupplierMessage(
        "Pickup location is required. Resolve address or pin map location.",
      );
      return;
    }
    ...
    if (!supplierPaymentProfile?.qrImageUrl) {
      setSupplierMessage(
        "Upload your supplier GPay QR in Payment Profile before publishing listings.",
      );
      return;
    }
    ...
    try {
      ...
      const listingFoodType: FoodType =
        saved.foodCategory === "non_veg" || saved.foodCategory === "seafood"
          ? "non_veg"
          : "veg";
  ...
  const submitEmergencySupplierListing = async () => {
    ...
    if (!supplierPaymentProfile?.qrImageUrl) {
      setSupplierMessage(
        "Upload your supplier GPay QR in Payment Profile before emergency circulation.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          payload?.error ?? "Unable to trigger emergency circulation.",
        );
      }
      ...
      setSupplierMessage(
        error instanceof Error
          ? error.message
          : "Unable to trigger emergency circulation.",
      );
    } finally {
  ...
  const submitBulkSupplierEvent = async () => {
    ...
    if (!supplierPaymentProfile?.qrImageUrl) {
      setSupplierMessage(
        "Upload your supplier GPay QR in Payment Profile before bulk publishing.",
      );
      return;
    }
    ...
    const invalidCookedAt = validItems.some((item) =>
      Number.isNaN(new Date(item.cookedAt).getTime()),
    );
    if (invalidCookedAt) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          payload?.error ?? "Unable to publish bulk donation event.",
        );
      }
      ...
      setSupplierMessage(
        error instanceof Error
          ? error.message
          : "Unable to publish bulk donation event.",
      );
    } finally {
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_5%_5%,#dcfce7_0%,#f8fafc_42%,#e0f2fe_100%)] text-slate-900">
        <section className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-4 py-10 text-center">
          ...
          <p className="mt-1 text-sm text-slate-600">
            Preparing the next page and session state.
          </p>
        </section>
      </main>
    );
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                  ...
                  <p className="mt-2 text-sm font-semibold text-slate-900">
                    Reduce food waste by connecting consumers and suppliers in
                    real time.
                  </p>
                  <p className="mt-1 text-xs text-amber-800">
                    Unified flows for order, publish, pickup, and delivery.
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:col-span-2">
                  <p className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-amber-300">
                    <BadgePercent className="size-3.5" /> Less than 50% market
                    price!
                  </p>
                  <p className="mt-2 text-xs text-slate-600 md:text-sm">
                    Suppliers publish discounted surplus, consumers reserve
                    instantly, and volunteers complete the final-mile pickup.
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-emerald-100 via-cyan-100 to-blue-100 text-slate-900 relative overflow-hidden">
      ...
      <div className="relative z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Food rescue & fairness marketplace
                </p>
              </div>
            ...
            <div>
              {trackingOrder ? (
                <div className="space-y-4">
                  <div className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">
                          After payment live view
                        </p>
                        <h2 className="text-2xl font-black text-slate-900">
                          Track your pickup route in real time
                        </h2>
                        <p className="text-sm text-slate-600">
                          Destination: {trackingOrder.listing.sellerName} for{" "}
                          {trackingOrder.listing.dish}
                        </p>
                      </div>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <p className="mt-1 text-xl font-black text-slate-900">
                          {typeof trackingDistanceKm === "number"
                            ? `${trackingDistanceKm.toFixed(2)} km`
                            : "Calculating..."}
                        </p>
                      </div>
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                        ...
                        <p className="mt-1 text-xl font-black text-slate-900">
                          {typeof trackingEtaMinutes === "number"
                    ...
                    <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50 p-4">
                      <p className="text-xs font-bold uppercase tracking-wide text-sky-700">
                        Inbound Redistribution Status
                      </p>
                      {isInboundTrackingLoading ? (
                        <p className="mt-1 text-xs text-sky-800">
                          Refreshing transit stage and suitability checks...
                        </p>
                      ) : inboundTracking ? (
                        <div className="mt-2 space-y-2 text-sm text-slate-700">
                          <p>
                            ...
                            {inboundTracking.stageLabel}
                          </p>
                          <p>
                            <span className="font-semibold text-slate-900">
                              Assigned volunteer:
                            </span>{" "}
                            {inboundTracking.assignedVolunteer.displayName}
                          </p>
                          <p>
                            <span className="font-semibold text-slate-900">
                              Pickup completed:
                            </span>{" "}
                            {inboundTracking.pickupCompleted
                              ? "Yes"
                              : "Pending"}
                          </p>
                          <p>
                            ...
                            {inboundTracking.suitability.spoilageRiskScore}/100
                          </p>
                          {inboundTracking.suitability.warnings.length ? (
                            ...
                            <p className="text-xs text-emerald-700">
                              No active suitability warning at the moment.
                            </p>
                          )}
                        </div>
                      ) : (
                        <p className="mt-1 text-xs text-slate-600">
                          Inbound telemetry will appear once route signals are
                          available.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <p className="text-sm font-semibold text-slate-900">
                        Supplier payment URL is visible only after pressing Pay
                      </p>
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        {paymentReferences.length ? (
                          paymentReferences.map((reference) => (
                            <div
                              key={`${reference.supplierId}-${reference.sellerName}`}
                              className="rounded-lg border border-slate-200 bg-white p-3"
                            >
                              <p className="text-sm font-bold text-slate-900">
                                {reference.sellerName}
                              </p>
                              ...
                              {reference.qrImageUrl ? (
                                ...
                                <p className="mt-2 text-xs text-slate-500">
                                  QR not available for this listing yet.
                                </p>
                              )}
                            </div>
                          ))
                          ...
                          <p className="text-xs text-slate-500">
                            No supplier payment references for this order.
                          </p>
                        )}
                      </div>
                    </div>
                      ...
                      {arrivedAtPickup ? (
                        reviewSubmitted ? (
                          <p className="text-sm font-semibold text-emerald-700">
                            Thanks for your review. Pickup marked completed.
                          </p>
                        ) : (
                          <div className="space-y-3">
                            <p className="text-sm font-semibold text-slate-900">
                              You have reached the location. Share your star
                              review.
                            </p>
                        ...
                        <p className="text-sm text-slate-600">
                          Live tracking active. Review unlocks automatically
                          once you arrive at the supplier location.
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm">
                    {!isNeedPostingOpen ? (
                      <Button
                        ...
                        onClick={() => setIsNeedPostingOpen(true)}
                      >
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              type="button"
                              variant="outline"
                              onClick={() => void loadReceiverNeedHistory()}
                            >
                              Refresh Needs
                            </Button>
                            <Button
                              type="button"
                              variant="outline"
                              onClick={() => setIsNeedPostingOpen(false)}
                            >
                              Close
                            </Button>
                          </div>
                        ...
                        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                          <Input
                            value={consumerNeedTitle}
                            onChange={(event) =>
                              setConsumerNeedTitle(event.target.value)
                            }
                          ...
                          <Input
                            type="number"
                            min={1}
                            value={consumerNeedMeals}
                            onChange={(event) =>
                              setConsumerNeedMeals(
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setConsumerNeedFoodPreference(
                                event.target
                                  .value as typeof consumerNeedFoodPreference,
                              )
                            }
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setConsumerNeedMealSlot(
                                event.target
                                  .value as typeof consumerNeedMealSlot,
                              )
                            }
                            className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm"
                          >
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setConsumerNeedUrgency(
                                event.target
                                  .value as typeof consumerNeedUrgency,
                              )
                            }
                            className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm"
                          >
                          ...
                          <Input
                            type="datetime-local"
                            value={consumerNeedWindowStart}
                            onChange={(event) =>
                              setConsumerNeedWindowStart(event.target.value)
                            }
                          />
                          <Input
                            type="datetime-local"
                            value={consumerNeedWindowEnd}
                            onChange={(event) =>
                              setConsumerNeedWindowEnd(event.target.value)
                            }
                          />
                          <Input
                            value={consumerNeedNote}
                            onChange={(event) =>
                              setConsumerNeedNote(event.target.value)
                            }
                            placeholder="Need note for suppliers"
                          />
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                            type="button"
                            onClick={() => void postConsumerNeed()}
                            disabled={isPostingConsumerNeed}
                          >
                            {isPostingConsumerNeed
                              ? "Posting need..."
                              : "Post Need and Trigger Matching"}
                          </Button>
                          <span className="text-xs text-slate-600">
                            Needs posted: {receiverNeedHistory.length}
                          </span>
                        </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    {consumerListings.map((listing) => {
                      ...
                      const recommendation = recommendationByListingId.get(
                        listing.id,
                      );
                      const scoreValue = recommendation?.score ?? 76;
                      const starCount = Math.max(
                        1,
                        Math.round(scoreValue / 20),
                      );
                      const sellerIsSolo = listing.sellerType === "individual";
                      return (
                        <article
                          key={listing.id}
                          className="flex flex-col h-full rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <div
                                className="flex items-center gap-1 rounded-full bg-white border border-slate-200 px-2 py-1 shadow-sm"
                                title={`Score ${scoreValue}/100`}
                              >
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <Star
                                    key={star}
                                    className={`size-4 ${
                                      star <= starCount
                                        ? "fill-amber-400 text-amber-300 drop-shadow-[0_0_8px_rgba(251,191,36,0.55)]"
                                        : "text-slate-600"
                                    }`}
                                  />
                                ))}
                          ...
                          <div className="mt-4 flex-1 flex items-start justify-between text-sm">
                            <div className="flex flex-col gap-2 items-start">
                              <span className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-sky-100 via-white to-cyan-50 px-3 py-2 font-semibold text-slate-900 shadow-sm">
                                <Navigation className="size-4 text-sky-700" />
                              ...
                              <span className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-200 via-amber-100 to-white px-3 py-2 font-semibold text-amber-900 shadow-sm">
                                <span className="text-lg font-black">
                                  Rs. {listing.unitPrice}
                                </span>
                            ...
                            <div className="flex flex-col gap-2 items-end">
                              <div
                                className={`group inline-flex items-center gap-1 rounded-xl px-3 py-2 font-semibold shadow-sm ${
                                  sellerIsSolo
                                    ? "bg-gradient-to-r from-emerald-100 via-white to-emerald-50 text-emerald-900"
                                    : "bg-gradient-to-r from-indigo-100 via-white to-indigo-50 text-indigo-900"
                                }`}
                                title={sellerIsSolo ? "Solo" : "Caterer"}
                                aria-label={
                                  sellerIsSolo ? "Solo seller" : "Caterer"
                                }
                              >
                                {sellerIsSolo ? (
                                ...
                                <span className="text-xs uppercase tracking-wide">
                                  {sellerIsSolo ? "Solo" : "Caterer"}
                                </span>
                              </div>
                              <span className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-100 via-white to-fuchsia-50 px-3 py-2 font-semibold text-slate-900 shadow-sm">
                                <ShoppingCart className="size-4 text-purple-700" />
                                <span className="text-xs uppercase text-slate-600">
                                  Delivery
                                </span>
                                <span className="text-sm font-bold">
                                  {listing.deliveryAvailable ? "Yes" : "Pickup"}
                                </span>
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="mt-auto pt-4 flex w-full items-center justify-between">
                            <p className="text-sm font-semibold text-slate-800">
                              Available: {available} {listing.unit}
                            </p>
                            {inCart > 0 ? (
                ...
                >
                  {/* Decorative elements */}
                  <div
                    className={`absolute right-0 top-0 -mr-16 -mt-16 h-32 w-32 rounded-full opacity-10 blur-3xl ${
                      supplierPublishMode === "emergency"
                        ? "bg-rose-300"
                        : supplierPublishMode === "bulk"
                          ? "bg-cyan-300"
                  ...
                  />
                  <div
                    className={`absolute left-0 bottom-0 -ml-16 -mb-16 h-32 w-32 rounded-full opacity-10 blur-3xl ${
                      supplierPublishMode === "emergency"
                        ? "bg-red-300"
                        : supplierPublishMode === "bulk"
                          ? "bg-sky-300"
                  ...
                  <div className="relative mb-5">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        <h2 className="text-xl font-black text-slate-900">
                          Publish your surplus food
                        </h2>
                        <p className="mt-1 text-sm text-slate-600">
                          Define listing details, upload payment QR, then choose
                          standard listing, emergency circulation, or bulk event
                          dispatch.
                        </p>
                      </div>
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-slate-600">
                      Upload your GPay/UPI QR image once. Buyers will see your
                      payment link only after confirming purchase.
                    </p>
                    ...
                    <div className="mt-3 flex flex-wrap items-end gap-3">
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => void saveSupplierQrProfile()}
                        disabled={isSavingSupplierQr}
                      >
                        {isSavingSupplierQr ? (
                      ...
                      <div className="mt-3 flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                            ✅ Payment QR is active
                          </p>
                          <p className="text-xs text-slate-600">
                            Buyers can pay using this QR after purchase.
                          </p>
                        </div>
                      </div>
                    ) : (
                      <p className="mt-3 text-xs text-rose-700 font-medium">
                        ⚠️ No QR uploaded yet. Save one before publishing
                        listings.
                      </p>
                    )}
                    ...
                    <div className="mt-3 space-y-2">
                      {isLoadingSupplierNeedPrompts ? (
                        ...
                        supplierNeedPrompts.slice(0, 5).map((prompt) => (
                          <article
                            key={prompt.id}
                            className="rounded-lg border border-blue-200 bg-white p-3 text-xs"
                          >
                            <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <p className="mt-1 text-slate-600">
                              ...
                              {prompt.distance_km != null
                                ...
                                : "unknown"}{" "}
                              • Avg qty {prompt.avg_quantity ?? "-"} • Recent
                      ...
                      <>
                        <label className="space-y-2 text-sm font-medium text-slate-700 md:col-span-2">
                          ...
                          <Input
                            ...
                            onChange={(event) =>
                              setSupplierBulkEventName(event.target.value)
                            }
                              ...
                              >
                                <div className="mb-2 flex items-center justify-between gap-2">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      removeSupplierBulkItem(item.id)
                                    }
                        ...
                        <label className="space-y-2 text-sm font-medium text-slate-700">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setSupplierForm((current) => ({
                                ...current,
                                packagingCondition: event.target
                                  .value as PackagingCondition,
                              }))
                            }
                            className="h-10 w-full rounded-md border border-slate-700 bg-slate-900 px-3 text-white"
                    ...
                    <label className="space-y-2 text-sm font-medium text-slate-700 md:col-span-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          type="button"
                          variant="outline"
                          disabled={isResolvingSupplierAddress}
                          onClick={() => void resolveSupplierPickupAddress()}
                        >
                      ...
                      {supplierForm.pickupLocation ? (
                        <p className="text-xs text-slate-600">
                          Selected: {supplierForm.pickupLocation.lat.toFixed(5)}
                          , {supplierForm.pickupLocation.lng.toFixed(5)}
                        </p>
                      ) : null}
                    </label>
                  </div>
                  ...
                  >
                    <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
                        ...
                        {supplierPublishMode === "standard" ? (
                          <div className="flex flex-col gap-1 w-full">
                            <Input
                              ...
                              className={`bg-white border-slate-300 w-full ${supplierPriceWarning ? "border-rose-400 focus-visible:ring-rose-400" : ""}`}
                            />
                            {isEvaluatingMaxPrice && (
                              <p className="text-[10px] text-slate-500 flex items-center gap-1 animate-pulse">
                                <Loader2 className="size-3 animate-spin" />{" "}
                                Evaluating market value...
                              </p>
                            )}
                            {supplierPriceWarning && !isEvaluatingMaxPrice && (
                              <p className="text-[10px] text-rose-600 font-semibold">
                                {supplierPriceWarning}
                              </p>
                            )}
                            {!supplierPriceWarning &&
                              ...
                              !isEvaluatingMaxPrice && (
                                <p className="text-[10px] text-emerald-600 font-semibold">
                                  Max allowed: Rs. {supplierMaxPrice.toFixed(0)}
                                </p>
                              )}
                          </div>
                        ) : supplierPublishMode === "emergency" ? (
                          ...
                          <Input
                            ...
                            onChange={(event) => {
                              const nextValue = Math.max(
                                30,
                                Math.min(360, Number(event.target.value) || 30),
                              );
                      ...
                      <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700">
                        {supplierPublishMode === "bulk" ? (
                          <div className="space-y-1">
                            <p className="font-semibold text-slate-900">
                              Bulk logistics preview
                            </p>
                            <p>
                              Each dish gets an individual spoilage score at
                              publish time.
                            </p>
                            <p>
                              Allocation picks one receiver if feasible,
                              otherwise splits by route-time and capacity.
                            </p>
                          </div>
                        ) : isSupplierRiskLoading ? (
                          <p className="inline-flex items-center gap-2">
                            <Loader2 className="size-3.5 animate-spin" />{" "}
                            Calculating live spoilage risk...
                          </p>
                        ) : supplierRiskPreview ? (
                          <div className="space-y-1">
                            <p className="font-semibold text-slate-900">
                              Live Risk Score: {supplierRiskPreview.score}/100
                            </p>
                            <p>
                              Status:{" "}
                              <span
                              ...
                              >
                                {supplierRiskPreview.label}
                              </span>
                            </p>
                            <p>
                              Recommended pickup window:{" "}
                              {
                                supplierRiskPreview.recommendedPickupWindowMinutes
                              }{" "}
                              min
                            </p>
                            <p>
                              Weather now:{" "}
                              {supplierRiskPreview.weather.temperatureC.toFixed(
                                1,
                              )}
                              C /{" "}
                              {Math.round(
                                supplierRiskPreview.weather.humidityPct,
                              )}
                              %
                            </p>
                            <p>
                              Route estimate:{" "}
                              {supplierRiskPreview.travel.durationMinutes} min (
                              {supplierRiskPreview.travel.distanceKm.toFixed(1)}{" "}
                              km)
                            </p>
                          </div>
                        ) : (
                          <p className="text-slate-600">
                            Complete food + location fields to see live spoilage
                            label.
                          </p>
                        )}
                      </div>
                    ...
                    {supplierPublishMode === "standard" ? (
                      <p className="mt-2 inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-amber-300">
                        <BadgePercent className="size-3.5" /> Less than 50%
                        market price!
                      </p>
                    ) : supplierPublishMode === "emergency" ? (
                      <p className="mt-2 rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700">
                        Emergency mode bypasses passive discovery and starts
                        priority dispatch to nearest feasible volunteers and
                        receivers by travel time.
                      </p>
                    ) : (
                      <p className="mt-2 rounded-md border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-semibold text-cyan-700">
                        Bulk mode groups many dishes under one parent event and
                        chooses either a single receiver or split allocations
                        based on route feasibility.
                      </p>
                    )}
                  </div>
                  ...
                  {supplierEmergencyResult ? (
                    <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm">
                      <p className="text-xs font-bold uppercase tracking-wide text-rose-700">
                        Urgent circulation status
                      </p>
                      <p className="mt-1 text-sm font-semibold text-slate-900">
                        Priority:{" "}
                        {supplierEmergencyResult.priorityLevel.toUpperCase()} •
                        State: {supplierEmergencyResult.priorityState}
                      </p>
                      <p className="mt-1 text-slate-700">
                        Expected response:{" "}
                        {supplierEmergencyResult.expectedResponseMinutes ??
                          "--"}{" "}
                        min • Window:{" "}
                        {supplierEmergencyResult.safeWindowMinutes} min
                      </p>
                      <p className="mt-2 text-xs text-slate-700">
                        Alerts: attempted{" "}
                        {supplierEmergencyResult.notification.attempted}, sent{" "}
                        {supplierEmergencyResult.notification.sent}, failed{" "}
                        {supplierEmergencyResult.notification.failed} (
                        {supplierEmergencyResult.notification.mode})
                      </p>
                  ...
                  {supplierBulkResult ? (
                    <div className="mt-4 rounded-xl border border-cyan-200 bg-cyan-50 p-4 text-sm">
                      <p className="text-xs font-bold uppercase tracking-wide text-cyan-700">
                        Bulk logistics status
                      </p>
                      <p className="mt-1 text-sm font-semibold text-slate-900">
                        Strategy: {supplierBulkResult.strategy} • State:{" "}
                        {supplierBulkResult.status}
                      </p>
                      <p className="mt-1 text-slate-700">
                        Window: {supplierBulkResult.safeWindowMinutes} min •
                        Expected response:{" "}
                        {supplierBulkResult.expectedResponseMinutes ?? "--"} min
                      </p>
                      <p className="mt-1 text-slate-700">
                        Total: {supplierBulkResult.totalQuantity} meals •
                        Unallocated: {supplierBulkResult.unallocatedQuantity}
                      </p>
                      {supplierBulkResult.assignedVolunteer ? (
                        <p className="mt-1 text-xs text-slate-700">
                          Volunteer:{" "}
                          {supplierBulkResult.assignedVolunteer.displayName} (
                          {supplierBulkResult.assignedVolunteer.etaMinutes} min)
                        </p>
                      ) : null}
                      <div className="mt-2 grid gap-2 md:grid-cols-2">
                        {supplierBulkResult.allocations.length ? (
                          supplierBulkResult.allocations.map((allocation) => (
                            <div
                              key={`${allocation.receiverId}-${allocation.receiverName}`}
                            ...
                            >
                              ...
                              <p className="text-xs text-slate-600">
                                {allocation.allocatedQuantity} meals • ETA{" "}
                                {allocation.etaMinutes} min •{" "}
                          ...
                          <p className="text-xs text-slate-600">
                            No receiver allocation could be made within the
                            current safe window.
                          </p>
                        )}
                      </div>
                  ...
                  <div className="mt-4 rounded-xl border border-slate-200 bg-white/80 p-4">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => void loadSupplierListings()}
                      >
                        Refresh
                      </Button>
                    </div>
                      ...
                      {supplierListings.length ? (
                        supplierListings.slice(0, 6).map((item) => (
                          ...
                          >
                              ...
                              <div className="flex flex-wrap items-center gap-1">
                                {item.isEmergency ? (
                                  <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2 py-1 font-bold text-rose-700">
                                    <AlertTriangle className="size-3.5" /> HIGH
                                    PRIORITY
                                  </span>
                                ) : null}
                                <span
                                ...
                                >
                                  {item.spoilageLabel === "Urgent Pickup" ? (
                            ...
                            <p className="mt-1 text-slate-600">
                              Score {item.spoilageScore}/100 • Pickup window{" "}
                              {item.recommendedPickupWindowMinutes} min • Qty{" "}
                              {item.quantity}
                            </p>
                            {item.isEmergency ? (
                              <p className="mt-1 text-slate-600">
                                State: {item.priorityState} • Expected response{" "}
                                {item.expectedResponseMinutes ?? "--"} min
                              </p>
                            ) : null}
                          </div>
                        ))
                        ...
                        <p className="text-xs text-slate-600">
                          No active supplier listings yet.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex justify-end">
                    <Button
                      ...
                      disabled={
                        isPublishingSupplierListing ||
                        isTriggeringEmergencyListing ||
                        isPublishingBulkEvent
                      }
                    >
                      {isPublishingSupplierListing ||
                      isTriggeringEmergencyListing ||
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-cyan-200 bg-gradient-to-br from-cyan-50 via-sky-50 to-blue-50 p-6 shadow-sm relative overflow-hidden">
                    ...
                    <div className="relative flex flex-wrap items-start justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase tracking-wider text-cyan-700">
                            Volunteer Control Room
                          </p>
                          <h2 className="text-2xl font-black text-slate-900">
                            Routing & Pickup Dashboard
                          </h2>
                          <p className="mt-1 text-sm text-slate-600">
                            Manage your live status, track nearby pickups, and
                            complete final-mile deliveries to earn rewards.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Button
                          variant="outline"
                          onClick={() => void loadVolunteerTaskFeed()}
                        >
                          Refresh Feed
                        </Button>
                      </div>
                    ...
                    <div className="relative mt-4 grid gap-2 sm:grid-cols-3">
                      ...
                      <label className="text-xs font-semibold text-slate-700">
                        ...
                        <Input
                          ...
                          onChange={(event) =>
                            setVolunteerPreferredZones(event.target.value)
                          }
                    ...
                    <div className="mt-5 grid gap-3 md:grid-cols-4">
                      <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
                        ...
                        <p className="mt-1 text-xs text-slate-600">
                          Your availability to accept tasks.
                        </p>
                      </div>
                      <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4">
                        <p className="text-sm font-bold text-slate-500 flex items-center gap-1">
                        ...
                        <p className="mt-1 text-xs text-slate-600">
                          Ranked tasks based on urgency, route, load-fit, and
                          impact.
                        </p>
                      </div>
                      <div className="rounded-xl border border-slate-200 bg-gradient-to-br from-amber-50 to-white p-4">
                        ...
                        <p className="mt-2 text-lg font-black text-slate-900">
                          {volunteerWeatherAdvisory
                            ? volunteerWeatherAdvisory.severity.toUpperCase()
                            : "UNKNOWN"}
                        </p>
                        <p className="mt-1 text-xs text-slate-600">
                          {volunteerWeatherAdvisory
                  ...
                  <div className="rounded-2xl border border-cyan-200 bg-white p-4 shadow-sm">
                    <p className="text-xs font-bold uppercase tracking-wide text-cyan-700">
                      Volunteer Workspace Focus
                    </p>
                    <p className="mt-1 text-sm text-slate-700">
                      Main page now prioritizes live dispatch and workflow
                      actions. Analytics are consolidated in the sidebar and
                      analytics dashboard.
                    </p>
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                      ...
                      <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                        ...
                        <p className="text-base font-bold text-slate-900">
                          {isVolunteerAvailable ? "Live" : "Paused"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {volunteerWeatherAdvisory ? (
                    <div className="rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-4 text-sm text-amber-900">
                      ...
                      <p className="mt-1">
                        {volunteerWeatherAdvisory.advisory}
                      </p>
                    </div>
                  ) : null}
                  ...
                  <div className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-sky-50 p-5 shadow-sm">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <h3 className="text-lg font-black text-slate-900">
                          Multi-stop pooled routes
                        </h3>
                        <p className="text-xs text-slate-600">
                          Suggested bundled pickups with stop order and staged
                          completion markers.
                        </p>
                      </div>
                    </div>
                    ...
                    {volunteerPooledTasks.length ? (
                          ...
                          >
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              ...
                              <div className="flex flex-wrap items-center gap-1 text-[11px]">
                                ...
                                <span className="rounded-full bg-slate-50 px-2 py-0.5 text-slate-700">
                                  Qty {pool.totalQuantity}
                                </span>
                                <span className="rounded-full bg-slate-50 px-2 py-0.5 text-slate-700">
                                  ETA {pool.estimatedTotalMinutes} min
                                </span>
                              </div>
                            </div>
                            <div className="mt-2 flex flex-wrap gap-1 text-[11px]">
                              {pool.stopOrder.map((stop) => (
                                <span
                                  key={`${pool.pooledTaskId}-${stop.listingId}`}
                                  className="rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-blue-800"
                                >
                                  Stop {stop.stopNumber}: {stop.supplierName} (
                                  {stop.quantity})
                                </span>
                              ))}
                            </div>
                      ...
                      <p className="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-600">
                        No pooled bundle recommended yet. Keep location and
                        capacity updated to unlock multi-stop opportunities.
                      </p>
                    )}
                  </div>
                    ...
                    <div className="relative mb-4 flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="mt-0.5 text-xs text-slate-600">
                          Assigned pickup tasks ready to start. Click
                          &quot;Route&quot; to begin navigation and earn points.
                        </p>
                        {isVolunteerTaskFeedLoading ? (
                          <p className="mt-1 text-[11px] text-cyan-700">
                            Refreshing intelligent task ranking...
                          </p>
                        ) : null}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      {volunteerQueue.length ? (
                        volunteerQueue.map((task) => (
                          <div
                            key={task.taskId}
                            className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-cyan-200 bg-gradient-to-r from-cyan-50 to-white p-3 hover:shadow-sm transition"
                          >
                            <div className="flex items-start gap-3 flex-1">
                              ...
                              <div>
                                ...
                                <div className="mt-1 flex flex-wrap items-center gap-1 text-[11px]">
                                  <span className="rounded-full bg-white px-2 py-0.5 font-semibold text-cyan-700">
                                    Score {task.score}/100
                                  </span>
                                  <span className="rounded-full bg-white px-2 py-0.5 text-slate-700">
                                    Pickup{" "}
                                    {task.route.volunteerToPickupKm.toFixed(1)}{" "}
                                    km
                                  </span>
                                  <span className="rounded-full bg-white px-2 py-0.5 text-slate-700">
                                    Total {task.route.totalMinutes} min
                                  </span>
                                  <span className="rounded-full bg-white px-2 py-0.5 text-slate-700">
                                    Qty {task.quantity}
                                  </span>
                                  <span className="rounded-full bg-white px-2 py-0.5 text-slate-700">
                                    Load-fit {task.reasons.loadFit}%
                                  </span>
                                  ...
                                  {task.reasons.tooHeavyForVehicle ? (
                                ...
                                <div className="mt-2 grid gap-2 md:grid-cols-4">
                                  <label className="text-[11px] font-semibold text-slate-700">
                                    ...
                                    <select
                                      ...
                                      onChange={(event) =>
                                        setVolunteerWorkflowStatusByTask(
                                          (current) => ({
                                            ...current,
                                            [task.taskId]: event.target
                                              .value as VolunteerWorkflowStatus,
                                          }),
                                        )
                                      }
                                  ...
                                  <label className="text-[11px] font-semibold text-slate-700 md:col-span-2">
                                    ...
                                    <Input
                                      ...
                                      onChange={(event) =>
                                        setVolunteerWorkflowNoteByTask(
                                          (current) => ({
                                            ...current,
                                  ...
                                  <label className="text-[11px] font-semibold text-slate-700">
                                    ...
                                    <input
                                      ...
                                      onChange={(event) => {
                                        ...
                                        setVolunteerWorkflowProofByTask(
                                          (current) => ({
                                            ...current,
                                            [task.taskId]: file,
                                          }),
                            ...
                            <div className="flex items-center gap-2">
                              <span className="rounded-full bg-cyan-100 px-3 py-1.5 text-xs font-bold text-cyan-700 inline-flex items-center gap-1">
                                ⏱️ {task.route.volunteerToPickupMinutes} min to
                                pickup
                              </span>
                              <Button
                                size="sm"
                                variant="outline"
                                disabled={Boolean(
                                  volunteerWorkflowBusyByTask[task.taskId],
                                )}
                                onClick={() =>
                                  void submitVolunteerWorkflowUpdate(task)
                                }
                              >
                                {volunteerWorkflowBusyByTask[task.taskId] ? (
                                  <Loader2 className="size-3.5 animate-spin" />
                        ...
                        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-center">
                          ...
                          <p className="text-sm text-slate-600">
                            No rescue tasks available right now.{" "}
                            <span className="font-semibold">
                              Set yourself Live
                            </span>{" "}
                            and wait for nearby pickups to appear!
                          </p>
                        </div>
                      )}
                    </div>
        ...
        <Dialog open={isFilterOpen} onOpenChange={setIsFilterOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Refine listings by price, distance, food type, seller type, and
                delivery.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <label className="space-y-1 text-sm font-medium text-slate-700">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setFoodTypeFilter(event.target.value as "all" | FoodType)
                  }
              ...
              <label className="space-y-1 text-sm font-medium text-slate-700">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setSellerTypeFilter(
                      event.target.value as "all" | SellerType,
                    )
                  }
                  className="h-9 w-full rounded-md border border-slate-300 bg-white px-3"
                >
              ...
              <label className="flex items-center gap-2 text-sm font-medium text-slate-700">
                <input
                  ...
                  onChange={(event) =>
                    setDeliveryOnlyFilter(event.target.checked)
                  }
                />
        ...
        <Dialog open={isMapOpen} onOpenChange={setIsMapOpen}>
          <DialogContent className="max-w-5xl p-3 md:p-5">
            <DialogHeader>
              ...
              <DialogDescription>
                Green markers are nearby food listings. Blue marker shows your
                current location.
              </DialogDescription>
            </DialogHeader>
        ...
        <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
          <SheetContent side="right" className="w-full sm:max-w-lg">
            <SheetHeader>
              ...
              <SheetDescription>
                Reserved items are reduced from available stock immediately to
                avoid conflicts.
              </SheetDescription>
            </SheetHeader>
              ...
              {cartItems.length ? (
                cartItems.map((item) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-600">
                      Rs. {item.listing.unitPrice} x {item.quantity}{" "}
                      {item.listing.unit}
                    </p>
                    <div className="mt-2 flex items-center justify-between">
                      <div className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-2 py-1">
                        <button
                          onClick={() => releaseListing(item.listing.id)}
                          className="rounded p-1 hover:bg-slate-100"
                        >
                          <Minus className="size-4" />
                        </button>
                        ...
                        <button
                          onClick={() => reserveListing(item.listing.id)}
                          className="rounded p-1 hover:bg-slate-100"
                        >
                          <Plus className="size-4" />
                        </button>
        ...
        <div
          className={`fixed right-6 z-50 space-y-3 ${
            !trackingOrder && mode === "consumer" ? "bottom-24" : "bottom-6"
          }`}
        >
          {isFloatingChatOpen ? (
            <div className="w-80 rounded-3xl border border-slate-200 bg-white/95 shadow-2xl">
              ...
              <div className="p-3">
                <WebsiteAiAssistant
                  title="Ask Feedo AI"
                  description="Quick answers about listings, routes, and pickups."
          ...
          <button
            type="button"
            onClick={() => setIsFloatingChatOpen((open) => !open)}
            className="group relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-emerald-300"
            aria-label="Open AI chat"
          >
            <Bot className="size-6 drop-shadow-sm" />
            <span className="pointer-events-none absolute -top-9 right-0 rounded-md bg-slate-900 px-2 py-1 text-[10px] font-semibold text-white opacity-0 transition group-hover:opacity-100">
              Chatbot
            </span>
          </button>
        </div>
